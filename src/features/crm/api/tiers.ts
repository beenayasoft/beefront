...
// Types pour la pagination Django REST Framework standard
...
// Types pour la pagination (version legacy - maintenue pour compatibilité)
...
// Fonction utilitaire pour convertir la réponse DRF en format legacy
export const convertDRFToPaginationInfo = (drfResponse: DRFPaginatedResponse<any>, currentPage: number, pageSize: number): PaginationInfo => {
  const totalPages = Math.ceil(drfResponse.count / pageSize);
  
  return {
    count: drfResponse.count,
    num_pages: totalPages,
    current_page: currentPage,
    page_size: pageSize,
    has_next: drfResponse.next !== null,
    has_previous: drfResponse.previous !== null,
    next_page: drfResponse.next ? currentPage + 1 : null,
    previous_page: drfResponse.previous ? currentPage - 1 : null,
  };
};
...
const adaptTierFromApi = (tierApi: any): Tier => {
  // Log de ce qui est reçu du backend
  console.log("🔍 adaptTierFromApi - Données reçues du backend:", tierApi);
  ...
  // Récupérer l'adresse principale (is_facturation=true ou première)
  const adressePrincipale = adresses.find(a => a?.is_facturation || a?.facturation) || adresses[0] || null;
  
  // Récupérer le contact principal
  const contactPrincipal = contacts.find(c => c?.is_contact_principal_devis || c?.contact_principal_devis) || contacts[0] || null;
  
  // Log détaillé pour débogage des valeurs extraites
  console.log("Adaptation tier:", {
    id: tierApi.id,
    idType: typeof tierApi.id,
    idToString: tierApi.id ? String(tierApi.id) : 'EMPTY',
    nom: tierApi.nom,
  ...
  // Debug: vérifier la structure de la réponse API
  console.log("🔍 DEBUG adaptTierFromApi - tierApi:", tierApi);
  console.log("🔍 DEBUG adaptTierFromApi - tierApi.id:", tierApi.id, `(type: ${typeof tierApi.id})`);
  
  const tier = {
    id: tierApi.id ? String(tierApi.id) : (console.warn("⚠️ ID manquant dans la réponse API!", tierApi), ''),
    name: tierApi.nom || '',
  ...
  console.log("🔍 DEBUG adaptTierFromApi - final tier.id:", tier.id);
...
export const tiersApi = {
  // Récupérer les tiers avec pagination (format DRF)
  getTiers: async (params: TiersFilters): Promise<PaginatedResponse<Tier>> => {
    ...
    if (params.type && params.type !== 'tous') {
      ...
      console.log(`🔍 Conversion du type: ${params.type}`);
    }
    ...
    const adaptedTiers = response.data.results.map(adaptTierFromApi);
    
    // Convertir la pagination DRF vers le format attendu
    const currentPage = params.page || 1;
    const pageSize = params.page_size || 10;
    const paginationInfo = convertDRFToPaginationInfo(response.data, currentPage, pageSize);
    
    const result: PaginatedResponse<Tier> = {
      results: adaptedTiers,
      pagination: paginationInfo
    };
