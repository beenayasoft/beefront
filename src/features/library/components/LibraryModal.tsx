...
import { EditorQuoteItem } from '@/features/documents/types';
...
export function LibraryModal({ open, onOpenChange, onSelect }: LibraryModalProps) {
  ...
  const convertToQuoteItem = (item: Work | Material | Labor): EditorQuoteItem => {
    const baseItem = {
      ...
      designation: item.name || 'Article sans nom',
    ...
    if ('components' in item) {
      ...
      return {
        ...
        unitPrice: item.recommendedPrice || 0,
            ...
            <TableRow>
              <TableCell colSpan={6} className="text-center py-8">
                <div className="flex items-center justify-center">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-Beenaya-600"></div>
            ...
            filteredWorks.map((work) => (
              <TableRow key={work.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                ...
                <TableCell className="font-semibold">
                  {formatCurrency(work.recommendedPrice || 0)} MAD
                </TableCell>
                <TableCell>
                  <Badge 
                    variant={(work.margin || 0) < 15 ? "destructive" : (work.margin || 0) < 25 ? "secondary" : "default"}
                    className="text-xs"
                  >
                    {(work.margin || 0).toFixed(1)}%
                  </Badge>
                </TableCell>
                <TableCell>
                  <Button
          ...
          {loading ? (
            ...
            filteredMaterials.map((material) => (
              <TableRow key={material.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                ...
                <TableCell className="font-semibold">
                  {formatCurrency(material.unitPrice || 0)} MAD
                </TableCell>
                <TableCell>
          ...
          {loading ? (
            ...
            filteredLabor.map((labor) => (
              <TableRow key={labor.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                ...
                <TableCell className="font-semibold">
                  {formatCurrency(labor.unitPrice || 0)} MAD
                </TableCell>
                <TableCell>
                  <Button
