/**
 * Modal d'aperçu unifiée pour les devis - Version html2pdf optimisée
...
export const QuotePreviewModal: React.FC<QuotePreviewModalProps> = ({
  ...
  const generateFilename = () => {
    const documentNumber = quote.number || quote.id;
    const clientName = quote.clientName?.replace(/[^a-zA-Z0-9]/g, '') || 'Client';
    const currentDate = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    return `devis-${documentNumber}_${clientName}_${currentDate}.pdf`;
  };
        ...
        <div className="flex items-center justify-between p-4 border-b bg-gray-50">
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-semibold text-gray-800">
              Aperçu du Devis N° {quote.number || 'Brouillon'}
            </h2>
          ...
          <div className="p-4 border-t bg-gray-50 text-sm text-gray-600">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <span><strong>Client:</strong> {quote.clientName || 'Non défini'}</span>
                <span><strong>Date:</strong> {quote.issueDate ? new Date(quote.issueDate).toLocaleDateString('fr-FR') : 'Non définie'}</span>
              </div>
              <div className="flex items-center gap-6">
                <span><strong>Total HT:</strong> {formatCurrency(Number(quote.totalHt) || 0)}</span>
                <span><strong>Total TTC:</strong> {formatCurrency(Number(quote.totalTtc) || 0)}</span>
              </div>
            </div>
          </div>
        )}
