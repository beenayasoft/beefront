/**
 * Composant de prévisualisation paginée pour les documents
 */
import React, { useMemo } from 'react';
import { formatCurrency } from '@/lib/utils';
import { formatDate } from '@/lib/utils/formatters';
import { paginateDocumentItems, type DocumentItem, type PaginationConfig } from '@/lib/utils/pagination';
import { PaymentMethodsSection } from '@/components/payment/PaymentMethodsSection';
import type { PaymentMethod } from '@/lib/api/paymentMethods';

interface PaginatedPreviewProps {
  document: {
    id: string;
    number: string;
    status: string;
    clientName: string;
    clientAddress?: string;
    projectName?: string;
    projectAddress?: string;
    issueDate: string;
    dueDate?: string;
    expiryDate?: string;
    items?: DocumentItem[];
    notes?: string;
    termsAndConditions?: string;
    totalHT: number;
    totalVAT: number;
    totalTTC: number;
    // Informations d'entreprise
    companyName?: string;
    companyAddress?: string;
    companyPhone?: string;
    companyEmail?: string;
    companyWebsite?: string;
    companySiret?: string;
    companyIce?: string;
    companyLogo?: string;
  };
  documentType: 'quote' | 'invoice';
  appearanceSettings?: {
    documentTemplate?: "modern" | "classic" | "minimal";
    primaryColor?: string;
    showLogo?: boolean;
    showCompanyName?: boolean;
    showCompanyAddress?: boolean;
    showCompanyEmail?: boolean;
    showCompanyPhone?: boolean;
    showCompanyWebsite?: boolean;
    showCompanySiret?: boolean;
    showCompanyIce?: boolean;
    showClientAddress?: boolean;
    showProjectInfo?: boolean;
    showNotes?: boolean;
    showPaymentTerms?: boolean;
    showBankDetails?: boolean;
    showSignatureArea?: boolean;
    tableHeaderColor?: string;
    tableAlternateColor?: string;
    // Nouveaux champs pour les moyens de paiement
    showPaymentMethods?: boolean;
    paymentMethodsTitle?: string;
    paymentMethodsLayout?: 'horizontal' | 'vertical' | 'grid';
    paymentMethodsStyle?: 'modern' | 'classic' | 'minimal';
  };
  paginationConfig?: PaginationConfig;
  paymentMethods?: PaymentMethod[];
}

const defaultAppearanceSettings = {
  documentTemplate: "modern" as const,
  primaryColor: "#1B333F",
  showLogo: true,
  showCompanyName: true,
  showCompanyAddress: true,
  showCompanyEmail: true,
  showCompanyPhone: true,
  showCompanyWebsite: true,
  showCompanySiret: true,
  showCompanyIce: true,
  showClientAddress: true,
  showProjectInfo: true,
  showNotes: true,
  showPaymentTerms: true,
  showBankDetails: true,
  showSignatureArea: true,
  tableHeaderColor: "#f8f9fa",
  tableAlternateColor: "#f2f2f2",
  // Configuration par défaut des moyens de paiement
  showPaymentMethods: true,
  paymentMethodsTitle: "Moyens de paiement",
  paymentMethodsLayout: "horizontal" as const,
  paymentMethodsStyle: "modern" as const,
};

const defaultPaginationConfig: PaginationConfig = {
  itemsPerPage: 15,
  keepChaptersIntact: true,
  maxLinesPerItem: 3
};

export const PaginatedPreview: React.FC<PaginatedPreviewProps> = ({
  document,
  documentType,
  appearanceSettings = defaultAppearanceSettings,
  paginationConfig = defaultPaginationConfig,
  paymentMethods = []
}) => {
  const settings = { ...defaultAppearanceSettings, ...appearanceSettings };

  // Paginer les items
  const pages = useMemo(() => {
    if (!document.items || document.items.length === 0) {
      return [{
        items: [],
        pageNumber: 1,
        totalPages: 1,
        hasHeader: true,
        hasFooter: true
      }];
    }
    return paginateDocumentItems(document.items, paginationConfig);
  }, [document.items, paginationConfig]);

  return (
    <div className="space-y-4">
      {pages.map((page, pageIndex) => (
        <div key={pageIndex} className="w-[210mm] bg-white shadow-lg mx-auto border border-gray-200 break-after-page">
          <div className="p-[20mm] text-[11px] leading-tight min-h-[297mm] relative">
            
            {/* En-tête - complet sur première page, simplifié sur les autres */}
            <DocumentHeader 
              document={document}
              documentType={documentType}
              settings={settings}
              isFirstPage={page.hasHeader}
            />

            {/* Informations client/projet - seulement sur première page */}
            {page.hasHeader && (
              <ClientProjectInfo 
                document={document}
                settings={settings}
              />
            )}

            {/* Tableau des éléments de cette page */}
            <ItemsTable 
              items={page.items}
              settings={settings}
              showHeaders={pageIndex === 0 || page.items.length > 0}
            />

            {/* Totaux - seulement sur la dernière page */}
            {pageIndex === pages.length - 1 && (
              <>
                <TotalsSection document={document} />
                
                {/* Notes et conditions - seulement sur dernière page */}
                <NotesAndConditions 
                  document={document}
                  settings={settings}
                />

                {/* Moyens de paiement - seulement sur dernière page */}
                {settings.showPaymentMethods && paymentMethods.length > 0 && (
                  <div className="mb-4">
                    <PaymentMethodsSection
                      paymentMethods={paymentMethods}
                      title={settings.paymentMethodsTitle}
                      layout={settings.paymentMethodsLayout}
                      style={settings.paymentMethodsStyle}
                      className="text-xs"
                    />
                  </div>
                )}
              </>
            )}

            {/* Pied de page avec numéro */}
            <PageFooter 
              pageNumber={page.pageNumber}
              totalPages={page.totalPages}
              settings={settings}
            />
          </div>
        </div>
      ))}
    </div>
  );
};

// Composants internes

const DocumentHeader: React.FC<{
  document: any;
  documentType: 'quote' | 'invoice';
  settings: any;
  isFirstPage: boolean;
}> = ({ document, documentType, settings, isFirstPage }) => (
  <div className={`flex justify-between items-start ${isFirstPage ? 'mb-6' : 'mb-4'}`}>
    <div>
      <h1 
        className={`${isFirstPage ? 'text-xl' : 'text-base'} font-bold`}
        style={{ color: settings.primaryColor }}
      >
        {documentType === 'invoice' ? 'FACTURE' : 'DEVIS'}
      </h1>
      <p className="text-gray-600">N° {document.number}</p>
      <p className="text-gray-600">Date: {formatDate(document.issueDate)}</p>
      {document.dueDate && (
        <p className="text-gray-600">Échéance: {formatDate(document.dueDate)}</p>
      )}
      {document.expiryDate && (
        <p className="text-gray-600">Validité: {formatDate(document.expiryDate)}</p>
      )}
    </div>
    
    <div className="text-right space-y-2">
      {/* Logo de l'entreprise */}
      {settings.showLogo && isFirstPage && (
        <div className="mb-3">
          {document.companyLogo ? (
            <img 
              src={document.companyLogo} 
              alt={document.companyName || "Logo entreprise"}
              className="h-16 w-auto ml-auto object-contain"
            />
          ) : (
            <div className="h-16 w-32 ml-auto border-2 border-dashed border-gray-300 flex items-center justify-center text-xs text-gray-500 rounded">
              Logo entreprise
            </div>
          )}
        </div>
      )}
      
      {/* Informations de l'entreprise */}
      <div>
        {settings.showCompanyName && (
          <p className="font-bold">{document.companyName || "Votre entreprise"}</p>
        )}
        {settings.showCompanyAddress && (document.companyAddress || "123 Rue de l'Exemple\n75000 Paris").split('\n').map((line: string, index: number) => (
          <p key={index}>{line}</p>
        ))}
        {settings.showCompanyEmail && (
          <p>{document.companyEmail || "contact@votre-entreprise.com"}</p>
        )}
        {settings.showCompanyPhone && (
          <p>{document.companyPhone || "01 23 45 67 89"}</p>
        )}
        {settings.showCompanyWebsite && document.companyWebsite && (
          <p className="text-xs text-gray-500">{document.companyWebsite}</p>
        )}
        {settings.showCompanySiret && document.companySiret && (
          <p className="text-xs text-gray-500">SIRET: {document.companySiret}</p>
        )}
        {settings.showCompanyIce && document.companyIce && (
          <p className="text-xs text-gray-500">ICE: {document.companyIce}</p>
        )}
      </div>
    </div>
  </div>
);

const ClientProjectInfo: React.FC<{
  document: any;
  settings: any;
}> = ({ document, settings }) => (
  <div className="grid grid-cols-2 gap-4 mb-6">
    {settings.showClientAddress && (
      <div className="border p-3 rounded">
        <h2 
          className="font-bold mb-1 text-xs"
          style={{ color: settings.primaryColor }}
        >
          Client
        </h2>
        <p>{document.clientName}</p>
        {document.clientAddress && <p>{document.clientAddress}</p>}
      </div>
    )}
    
    {settings.showProjectInfo && (document.projectName || document.projectAddress) && (
      <div className="border p-3 rounded">
        <h2 
          className="font-bold mb-1 text-xs"
          style={{ color: settings.primaryColor }}
        >
          Projet
        </h2>
        {document.projectName && <p>{document.projectName}</p>}
        {document.projectAddress && <p>{document.projectAddress}</p>}
      </div>
    )}
  </div>
);

const ItemsTable: React.FC<{
  items: DocumentItem[];
  settings: any;
  showHeaders: boolean;
}> = ({ items, settings, showHeaders }) => (
  <table className="w-full mb-6 text-xs">
    {showHeaders && (
      <thead style={{ backgroundColor: settings.tableHeaderColor }}>
        <tr>
          <th className="px-2 py-1 text-left font-medium text-gray-500 uppercase tracking-wider">Désignation</th>
          <th className="px-2 py-1 text-right font-medium text-gray-500 uppercase tracking-wider">Qté</th>
          <th className="px-2 py-1 text-right font-medium text-gray-500 uppercase tracking-wider">PU HT</th>
          <th className="px-2 py-1 text-right font-medium text-gray-500 uppercase tracking-wider">TVA</th>
          <th className="px-2 py-1 text-right font-medium text-gray-500 uppercase tracking-wider">Total HT</th>
        </tr>
      </thead>
    )}
    <tbody className="divide-y divide-gray-200">
      {items.map((item, index) => (
        <tr 
          key={item.id} 
          className={item.type === 'chapter' ? 'font-bold' : ''}
          style={{
            backgroundColor: item.type === 'chapter' 
              ? settings.tableHeaderColor 
              : index % 2 === 1 
                ? settings.tableAlternateColor 
                : 'transparent'
          }}
        >
          <td className="px-2 py-1 whitespace-normal">
            <div className="font-medium">{item.designation}</div>
            {item.description && <div className="text-xs text-gray-500">{item.description}</div>}
          </td>
          <td className="px-2 py-1 text-right">{item.quantity}</td>
          <td className="px-2 py-1 text-right">{formatCurrency(item.unitPrice)}</td>
          <td className="px-2 py-1 text-right">20%</td>
          <td className="px-2 py-1 text-right font-medium">{formatCurrency(item.totalHT)}</td>
        </tr>
      ))}
    </tbody>
  </table>
);

const TotalsSection: React.FC<{ document: any }> = ({ document }) => (
  <div className="flex justify-end mb-6">
    <div className="w-48">
      <div className="flex justify-between py-1 text-xs">
        <span>Total HT</span>
        <span className="font-medium">{formatCurrency(document.totalHT)}</span>
      </div>
      <div className="flex justify-between py-1 text-xs">
        <span>TVA</span>
        <span className="font-medium">{formatCurrency(document.totalVAT)}</span>
      </div>
      <div className="flex justify-between py-1 border-t border-gray-200 font-bold text-xs">
        <span>Total TTC</span>
        <span>{formatCurrency(document.totalTTC)}</span>
      </div>
    </div>
  </div>
);

const NotesAndConditions: React.FC<{
  document: any;
  settings: any;
}> = ({ document, settings }) => (
  <>
    {settings.showNotes && document.notes && (
      <div className="mb-4 text-xs">
        <h3 
          className="font-bold mb-1"
          style={{ color: settings.primaryColor }}
        >
          Notes
        </h3>
        <p className="text-gray-700">{document.notes}</p>
      </div>
    )}
    
    {settings.showPaymentTerms && document.termsAndConditions && (
      <div className="mb-4 text-xs">
        <h3 
          className="font-bold mb-1"
          style={{ color: settings.primaryColor }}
        >
          Conditions générales
        </h3>
        <p className="text-gray-700">{document.termsAndConditions}</p>
      </div>
    )}
  </>
);

const PageFooter: React.FC<{
  pageNumber: number;
  totalPages: number;
  settings: any;
}> = ({ pageNumber, totalPages, settings }) => (
  <div className="absolute bottom-4 right-4 text-xs text-gray-500">
    Page {pageNumber} / {totalPages}
  </div>
);

export default PaginatedPreview;