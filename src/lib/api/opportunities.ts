...
const adaptOpportunityFromApi = (opportunityApi: any): Opportunity => {
  ...
  return {
    ...
    tierName: opportunityApi.tier_name || opportunityApi.tierName || '',
...
const adaptOpportunityToApi = (opportunity: Partial<Opportunity>): any => {
  ...
  if (opportunity.assignedTo) {
    // Si la valeur est "none", définir explicitement à null
    if (opportunity.assignedTo === "none") {
      assignedTo = null;
    } else {
  ...
  const apiData = {
    ...
    tier: opportunity.tierId, // Le backend attend 'tier', pas 'tierId'
...
export const opportunitiesApi = {
  ...
  getOpportunities: async (filters?: OpportunityFilters): Promise<Opportunity[]> => {
    try {
      ...
      console.log('📞 Appel API: GET /opportunites/ avec filtres:', filters);
      
      const response = await apiClient.get('/opportunites/', { params: filters });
  ...
  getOpportunity: async (id: string): Promise<Opportunity> => {
    try {
      console.log(`📞 Appel API: GET /opportunites/${id}/`);
  ...
  createOpportunity: async (data: Partial<Opportunity>): Promise<Opportunity> => {
    try {
      ...
      console.log('📞 Appel API: POST /opportunites/ avec données:', data);
      ...
      console.log(' - tier:', apiData.tier ? '✅' : '❌');
      console.log(' - estimated_amount:', apiData.estimated_amount ? '✅' : '❌');
      console.log(' - expected_close_date:', apiData.expected_close_date ? '✅' : '❌');
      
      const response = await apiClient.post('/opportunites/', apiData);
      return adaptOpportunityFromApi(response.data);
    } catch (error) {
  ...
  updateOpportunity: async (id: string, data: Partial<Opportunity>): Promise<Opportunity> => {
    try {
      console.log(`📞 Appel API: PATCH /opportunites/${id}/ avec données:`, data);
      const apiData = adaptOpportunityToApi(data);
      const response = await apiClient.patch(`/opportunites/${id}/`, apiData);
      return adaptOpportunityFromApi(response.data);
    } catch (error) {
  ...
  deleteOpportunity: async (id: string): Promise<void> => {
    try {
      console.log(`📞 Appel API: DELETE /opportunites/${id}/`);
      await apiClient.delete(`/opportunites/${id}/`);
    } catch (error) {
  ...
  updateOpportunityStage: async (id: string, stage: string): Promise<Opportunity> => {
    try {
      console.log(`📞 Appel API: PATCH /opportunites/${id}/update_stage/ avec stage:`, stage);
      const response = await apiClient.patch(`/opportunites/${id}/update_stage/`, { stage });
      return adaptOpportunityFromApi(response.data);
    } catch (error) {
  ...
  markAsWon: async (id: string, data?: { project_id?: string }): Promise<Opportunity> => {
    try {
      console.log(`📞 Appel API: POST /opportunites/${id}/mark_won/ avec données:`, data || {});
      const response = await apiClient.post(`/opportunites/${id}/mark_won/`, data || {});
  ...
  markAsLost: async (id: string, data: { loss_reason: string; loss_description?: string }): Promise<Opportunity> => {
    try {
      console.log(`📞 Appel API: POST /opportunites/${id}/mark_lost/ avec données:`, data);
      const response = await apiClient.post(`/opportunites/${id}/mark_lost/`, data);
      return adaptOpportunityFromApi(response.data.opportunity || response.data);
    } catch (error) {
  ...
  getKanbanData: async (): Promise<any> => {
    try {
      console.log('📞 Appel API: GET /opportunites/kanban/');
      const response = await apiClient.get('/opportunites/kanban/');
  ...
  getOpportunityStats: async (): Promise<any> => {
    try {
      console.log('📞 Appel API: GET /opportunites/stats/');
      const response = await apiClient.get('/opportunites/stats/');
      return response.data;
    } catch (error) {
