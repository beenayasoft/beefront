...
import { OpportunityStatsPOC } from "@/components/opportunities/OpportunityStatsPOC";
import { useOpportunitiesStatsInvalidation } from "@/hooks/useOpportunitiesStatsPOC";
...
export default function Opportunities() {
  ...
  // 🚀 POC: Hook pour invalidation automatique des stats
  const { invalidateStats } = useOpportunitiesStatsInvalidation();
  ...
  useEffect(() => {
    const loadOpportunities = async () => {
      try {
        ...
        // 🚀 POC: Invalidation automatique des stats après chargement initial
        console.log("🎯 POC: Chargement initial réussi, invalidation des stats...");
        invalidateStats();
  ...
  const handleStageChange = async (opportunity: Opportunity, newStage: OpportunityStatus) => {
    try {
      ...
      // 🚀 OPTIMISATION DRAG & DROP: Recharger toutes les opportunités pour garantir la synchronisation
      console.log("🔄 Rechargement des opportunités après drag & drop...");
      const refreshedOpportunities = await opportunityService.getOpportunities();
      setOpportunities(refreshedOpportunities);
      
      // 🚀 POC: Invalidation automatique des stats après changement de stage
      console.log("🎯 POC: Changement de stage réussi, invalidation des stats...");
      invalidateStats();
    ...
    if (confirm(`Êtes-vous sûr de vouloir supprimer l'opportunité "${opportunity.name}" ?`)) {
      try {
        ...
        if (success) {
          ...
          // 🚀 POC: Invalidation automatique des stats après suppression
          console.log("🎯 POC: Suppression réussie, invalidation des stats...");
          invalidateStats();
  ...
  const handleMarkAsWon = async (opportunity: Opportunity) => {
    try {
      ...
      // 🚀 POC: Invalidation automatique des stats après marquage gagnée
      console.log("🎯 POC: Marquage gagnée réussi, invalidation des stats...");
      invalidateStats();
  ...
  const handleConfirmLoss = useCallback(async (data: { lossReason: LossReason; lossDescription?: string }) => {
    ...
    try {
      ...
      // 🚀 POC: Invalidation automatique des stats après marquage perdue
      console.log("🎯 POC: Marquage perdue réussi, invalidation des stats...");
      invalidateStats();
  ...
  }, [opportunityToLose, resetLossFormState, isProcessing, invalidateStats]);
  ...
  const handleFormSubmit = useCallback(async (formData: Partial<Opportunity>) => {
    ...
    try {
      ...
      // 🚀 POC: Invalidation automatique des stats après création/modification
      console.log("🎯 POC: Création/modification réussie, invalidation des stats...");
      invalidateStats();
  ...
  }, [editingOpportunity, resetFormState, isSubmitting, invalidateStats]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Stats - POC React Query */}
      <OpportunityStatsPOC showMetadata={true} />
      
      {/* Stats originales (désactivées pour le POC) */}
